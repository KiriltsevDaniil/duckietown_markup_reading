...
k = 1
signs = []
legend = ["name", "width", "height", "class"] #2,3 new set; 1, 4 old set;
new_data = []
...
while i < len(data):
    for key in data["regions"][i].keys(): #collecting signs
        signs.append(key)
        
    for sign in signs:
        if f'({k}){data["filename"][i]}' not in forbidden:
            image = Image.open(f'./new_dataset/({k}){data["filename"][i]}', "r") #openning new image
            w, h = image.size
